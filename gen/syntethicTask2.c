...
void G2_pre_load_tasks(){
	...
	G2_data[0]=hgr_load_mem(100,"B",1,32);
 	
	wcet_per_task[1]=10.91;
	G2_data[1]=hgr_load_mem(100,"B",1,32);
 	
	wcet_per_task[2]=10.82;
	G2_data[2]=hgr_load_mem(100,"B",1,32);
 	
	wcet_per_task[3]=10.12;
	G2_data[3]=hgr_load_mem(100,"B",1,32);
 	
	wcet_per_task[4]=10.86;
	G2_data[4]=hgr_load_mem(100,"B",1,32);
...
void *G2_1(){
	...
	hgr_compute_sparse_task(G2_data[0],G2_cycle_per_task[0]); //Mem & Computation phase
	
	hgr_release_dependency(&G2_1_2); //release dependency
	hgr_release_dependency(&G2_1_3); //release dependency
	hgr_exit();
}
void *G2_2(){
	printf("Hi im the node G2_2\n");  //2 [miet="10.21", meet="10.91", maet="10.34", mem="100", unit="b"]

	hgr_wait_dependency(&G2_1_2); //wait dependency
	
	hgr_compute_sparse_task(G2_data[1],G2_cycle_per_task[1]); //Mem & Computation phase
	
	hgr_release_dependency(&G2_2_4); //release dependency
	hgr_exit();
}
void *G2_3(){
	printf("Hi im the node G2_3\n");  //3 [miet="10.42", meet="10.82", maet="10.66", mem="100", unit="b"]

	hgr_wait_dependency(&G2_1_3); //wait dependency
	
	hgr_compute_sparse_task(G2_data[2],G2_cycle_per_task[2]); //Mem & Computation phase
	
	hgr_release_dependency(&G2_3_4); //release dependency
	hgr_exit();
}
void *G2_4(){
	printf("Hi im the node G2_4\n");  //4 [miet="10.38", meet="10.12", maet="10.89", mem="100", unit="b"]

	hgr_wait_dependency(&G2_2_4); //wait dependency
	hgr_wait_dependency(&G2_3_4); //wait dependency
	hgr_wait_dependency(&G2_5_4); //wait dependency
	
	hgr_compute_sparse_task(G2_data[3],G2_cycle_per_task[3]); //Mem & Computation phase
...
void *G2_5(){
	...
	hgr_compute_sparse_task(G2_data[4],G2_cycle_per_task[4]); //Mem & Computation phase
	
	hgr_release_dependency(&G2_5_4); //release dependency
	hgr_exit();
}
